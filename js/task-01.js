...
for (let index = 0; index < liItemEl.length; index++) {
    const element = liItemEl[index];
    const title = element.querySelector(".title")
    console.log('Категория:', title.textContent);
    const navItemEl = element.querySelectorAll("ul");
    console.log('Количество элементов:',navItemEl[0].children.length);
}
